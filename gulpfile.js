...
var Context = {
  ...
  base_url: "https://kb.myetherwallet.com/",
...
function notifyFunc(msg) {
  return gulp.src(".", { read: false }).pipe(notify(onSuccess(msg)));
}

// Copy
gulp.task("copy", function() {
  gulp.src(img_srcFolder + "**/*/").pipe(gulp.dest(dstFolder + "images/"));

  gulp.src(srcFolder + "sitemap.xml").pipe(gulp.dest(dstFolder));

  return gulp
    .src(srcFolder + "styleguide.html")
...
gulp.task("styles", function() {
  return gulp
    .src(style_srcFolder + style_srcFile)

    .pipe(plumber({ errorHandler: onError }))
    .pipe(sass())
    .pipe(
      autoprefixer({ browsers: ["last 4 versions", "iOS > 7"], remove: false })
    )
...
gulp.task("scripts", function() {
  return gulp
    .src([
      scripts_srcFolder + "/languages/**/*.js",
      scripts_srcFolder + "**/*.js"
    ])
    .pipe(plumber({ errorHandler: onError }))
...
gulp.task("layout_home", ["register_partials"], function() {
  return gulp
    .src(layouts_srcFolder + "index.hbs")
    .pipe(plumber({ errorHandler: onError }))
    .pipe(
      tap(function(file, t) {
...
gulp.task("layout_search", ["register_partials"], function() {
  return gulp
    .src(layouts_srcFolder + "search.hbs")
    .pipe(plumber({ errorHandler: onError }))
    .pipe(
      tap(function(file, t) {
        H.registerHelpers(Handlebars);
...
gulp.task("layout_contact", ["register_partials"], function() {
  return gulp
    .src(layouts_srcFolder + "contact.hbs")
    .pipe(plumber({ errorHandler: onError }))
    .pipe(
      tap(function(file, t) {
        H.registerHelpers(Handlebars);
...
gulp.task("layout_form", ["register_partials"], function() {
  return gulp
    .src(layouts_srcFolder + "form.hbs")
    .pipe(plumber({ errorHandler: onError }))
    .pipe(
      tap(function(file, t) {
        H.registerHelpers(Handlebars);
...
gulp.task("layout_cats", ["register_partials"], function() {
  return gulp
    .src(layouts_srcFolder + "category.hbs")
    .pipe(plumber({ errorHandler: onError }))
    .pipe(
      tap(function(file) {
        H.registerHelpers(Handlebars);
...
gulp.task("layout_single", ["register_partials"], function() {
  return gulp
    .src(layouts_srcFolder + "single.hbs")
    .pipe(plumber({ errorHandler: onError }))
    .pipe(
      tap(function(file) {
        H.registerHelpers(Handlebars);
...
gulp.task("watch_js", function() {
  gulp.watch(scripts_srcFolder + "**/*.js", ["scripts"]);
});

gulp.task("watch_styles", function() {
  gulp.watch(style_srcFolder + "**/*.scss", ["styles"]);
});

gulp.task("watch_content", function() {
  gulp.watch(srcFolder + "**/*.md", [
    "layout_single",
    "layout_home",
    "layout_contact",
    "layout_search",
